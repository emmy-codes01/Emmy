...
const Blog = () => {
  ...
  // Keep the open detail view in sync whenever the realtime listener above
  // delivers a fresh blogPosts array (e.g. someone else's like lands),
  // since selectedPost is otherwise a frozen snapshot taken at click time.
  useEffect(() => {
    if (selectedPost) {
      const updated = blogPosts.find(p => p.id === selectedPost.id);
  ...
  // ScrollReveal() itself was being called fresh every time this ran, which
  // creates a brand-new instance with no memory of what the *previous*
  // instance already revealed — so already-visible cards could get reset
  // and replayed. Keeping one instance alive in a ref fixes that; only new
  // cards get animated in, existing ones are left alone.
  ...
  useEffect(() => {
    ...
    if (!isLoading && !selectedPost) {
      srRef.current.reveal('.post-reveal', {
        distance: '20px',
        duration: 700,
        interval: 80,
  ...
  // These no longer do their own optimistic setBlogPosts/setSelectedPost
  // writes, and no longer re-fetch the doc with getDoc before writing.
  // blogPosts (from the onSnapshot listener) is now the single source of
  // truth — the manual writes here were a second, independent source that
  // could momentarily disagree with what the listener delivered a moment
  // later, which read as another flavor of "glitching" (counts/icons
  // flickering between two values right after a click).
  const handleLikePost = async (postId) => {
    const post = blogPosts.find(p => p.id === postId) || (selectedPost?.id === postId ? selectedPost : null);
    if (!post) {
    ...
    try {
      ...
      } else if (post.dislikedByCurrentUser) {
        await updateDoc(postRef, {
          likes: increment(1),
